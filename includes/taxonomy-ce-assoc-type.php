...
function wpct_ce_lock_assoc_type_taxonomy()
{
    $options = array_map(function ($option) {
        return $option['slug'];
    }, WPCT_CE_REST_ASSOC_TYPE_TERMS);

    $terms = get_terms([
        'hide_empty' => false,
        'taxonomy' => WPCT_CE_REST_ASSOC_TYPE_TAX
    ]);

    foreach ($terms as $term) {
        if (!in_array($term->slug, $options)) {
            wp_delete_term($term->term_id, WPCT_CE_REST_ASSOC_TYPE_TAX);
        }
    }
}

add_action('init', 'wpct_ce_register_assoc_type_terms', 30);
function wpct_ce_register_assoc_type_terms()
{
    if (defined('WP_CLI') && WP_CLI) {
        return;
    }
